...
public class WeekView extends View {
    ...
    //ljt modify
    private Paint mStartTimeLinePaint;
    private Paint mEndTimeLinePaint;
    ...
    private SelectedDayOfMonthListener mSelectedDayOfMonthListener;
    private boolean isEventLongPress=false;
    private boolean isEmptyLongPress=false;
    private boolean startOrEnd=false;
    private int isStartOrEndTime=-1; //0 is startTime,1 is endTime
    private static final int START_TIME=0;
    private static final int END_TIME=1;
    //ljt modify
    private Calendar mEndMoveLineTop;
    private Calendar mStartMoveLineTop;
    private boolean isDrawTimeLine=false;
    private EventRect currentSelectEventRect;
    private int selectedDayOfMonth;
    private boolean isDown=false;
    private boolean isFirstDrawHeader=true;
...
//                    break;
        ...
        public void onLongPress(MotionEvent e) {
            ...
            if (mEventLongPressListener != null && mEventRects != null) {
                ...
                for (EventRect event : reversedEventRects) {
                    if (event.rectF != null && e.getX() > event.rectF.left && e.getX() < event.rectF.right && e.getY() > event.rectF.top && e.getY() < event.rectF.bottom) {
                        isEventLongPress=true;
                        if(e.getY()<=(event.rectF.top+event.rectF.bottom)/2) {
                            isStartOrEndTime=START_TIME;
                        } else {
                            isStartOrEndTime=END_TIME;
                        }
                        performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
                        currentSelectEventRect=event;
                        return;
                    }
                }
            }
...
//            }
            ...
            if (mEmptyViewLongPressListener != null && e.getX() > mHeaderColumnWidth && e.getY() > (mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
                isEmptyLongPress=true;
                Calendar selectedTime = getTimeFromPoint(e.getX(), e.getY());
                if (selectedTime != null) {
                    if (startOrEnd){
                        mEndMoveLineTop = selectedTime;
                    } else {
                        mStartMoveLineTop = selectedTime;
                    }
                    mEmptyViewLongPressListener.onEmptyViewLongPress(selectedTime,startOrEnd);
                    performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
                }
            }
//            // If the tap was on in an empty space, then trigger the callback.
...
//            moveLineTop=e.getY();
    ...
    private void init() {
        ...
        //ljt modify
        mStartTimeLinePaint= new Paint();
        mStartTimeLinePaint.setStrokeWidth((float)10);
        mStartTimeLinePaint.setColor(getResources().getColor(R.color.event_color_04));

        mEndTimeLinePaint= new Paint();
        mEndTimeLinePaint.setStrokeWidth((float)10);
        mEndTimeLinePaint.setColor(getResources().getColor(R.color.event_color_02));
...
//        drawCurrentTimeLine(canvas);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        //ljt modify
        if(!isFirstDrawHeader)
            today.set(Calendar.DATE,selectedDayOfMonth);
...
//                float startY = mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight/2 + mHeaderMarginBottom + mCurrentOrigin.y;
            ...
            //ljt modify
            drawCurrentTimeLine(canvas, start, startPixel, startY);
        ...
        boolean TueDay=false;
        boolean WedDay=false;
        boolean ThuDay=false;
        boolean FriDay=false;
        boolean SatDay=false;
        boolean SunDay=false;

        switch (today.getTime().getDate()) {
            case 2:
                MonDay=true;
                break;
            case 3:
...
//            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        //ljt modify
        canvas.drawText("MON", startPixel+1*(mWidthPerDay-2*startPixel) / 7, mHeaderTextHeight + mHeaderRowPadding, MonDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        canvas.drawText("TUE", startPixel+2*(mWidthPerDay-2*startPixel) / 7, mHeaderTextHeight + mHeaderRowPadding, TueDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        canvas.drawText("WED", startPixel+3*(mWidthPerDay-2*startPixel) / 7, mHeaderTextHeight + mHeaderRowPadding, WedDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        canvas.drawText("THU", startPixel+4*(mWidthPerDay-2*startPixel) / 7, mHeaderTextHeight + mHeaderRowPadding, ThuDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        canvas.drawText("FRI", startPixel+5*(mWidthPerDay-2*startPixel) / 7, mHeaderTextHeight + mHeaderRowPadding, FriDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        canvas.drawText("SAT", startPixel+6*(mWidthPerDay-2*startPixel) / 7  , mHeaderTextHeight + mHeaderRowPadding, SatDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
        canvas.drawText("SUN", startPixel+7*(mWidthPerDay-2*startPixel) / 7 , mHeaderTextHeight + mHeaderRowPadding, SunDay ? mTodayHeaderTextPaint : mHeaderTextPaint);

        drawAllDayEvents(day, startPixel, canvas);
    }

    //ljt modify
    private void drawCurrentTimeLine(Canvas canvas, float start, float startPixel, float startY) {
        if(mStartMoveLineTop != null) {
            float beforeNow = (mStartMoveLineTop.get(Calendar.HOUR_OF_DAY) + mStartMoveLineTop.get(Calendar.MINUTE)/60.0f) * mHourHeight;
            canvas.drawLine(start, startY + beforeNow, startPixel + mWidthPerDay, startY + beforeNow ,mStartTimeLinePaint);
        }
        if (mEndMoveLineTop != null) {
            float beforeNow = (mEndMoveLineTop.get(Calendar.HOUR_OF_DAY) + mEndMoveLineTop.get(Calendar.MINUTE)/60.0f) * mHourHeight;
            canvas.drawLine(start, startY + beforeNow, startPixel + mWidthPerDay, startY + beforeNow,mEndTimeLinePaint);
        }
    }
    ...
    private Calendar getTimeFromPoint(float x, float y){
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0 && x > start && x < startPixel + mWidthPerDay){
                ...
                // ljt modify
                if(!isFirstDrawHeader)
                    day.set(Calendar.DATE,selectedDayOfMonth);
...
//                sortAndCacheEvents(nextPeriodEvents);
    ...
    public void setEventDrawListener (EventDrawListener eventDrawListener) {
        this.mEventDrawListener = eventDrawListener;
    }

    public void setSelectDayOfWeekListener (SelectedDayOfMonthListener selectedDayOfMonthListener) {
        this.mSelectedDayOfMonthListener = selectedDayOfMonthListener;
    }
...
//                        SimpleDateFormat sdf = DateFormat.is24HourFormat(getContext()) ? new SimpleDateFormat("HH:mm", Locale.getDefault()) : new SimpleDateFormat("hh a", Locale.getDefault());
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if(event.getY()>0 && event.getY()<mHeaderTextHeight + 2*mHeaderRowPadding) {
                    if(event.getX()>122+mHeaderRowPadding && event.getX()<122+mHeaderRowPadding+1*(mWidthPerDay-2*122)/7) {
                        isDown=true;
                        selectedDayOfMonth=2;
                    } else if(event.getX()>122+mHeaderRowPadding+1*(mWidthPerDay-2*122)/7 && event.getX()<122+mHeaderRowPadding+2*(mWidthPerDay-2*122)/7) {
                        isDown=true;
                        selectedDayOfMonth=3;
                    } else if(event.getX()>122+mHeaderRowPadding+2*(mWidthPerDay-2*122)/7 && event.getX()<122+mHeaderRowPadding+3*(mWidthPerDay-2*122)/7) {
                        isDown=true;
                        selectedDayOfMonth=4;
                    } else if(event.getX()>122+mHeaderRowPadding+3*(mWidthPerDay-2*122)/7 && event.getX()<122+mHeaderRowPadding+4*(mWidthPerDay-2*122)/7) {
                        isDown=true;
                        selectedDayOfMonth=5;
                    } else if(event.getX()>122+mHeaderRowPadding+4*(mWidthPerDay-2*122)/7 && event.getX()<122+mHeaderRowPadding+5*(mWidthPerDay-2*122)/7) {
                        isDown=true;
                        selectedDayOfMonth=6;
                    } else if(event.getX()>122+mHeaderRowPadding+5*(mWidthPerDay-2*122)/7 && event.getX()<122+mHeaderRowPadding+6*(mWidthPerDay-2*122)/7) {
                        isDown=true;
                        selectedDayOfMonth=7;
                    } else if(event.getX()>122+mHeaderRowPadding+6*(mWidthPerDay-2*122)/7 && event.getX()<122+mHeaderRowPadding+7*(mWidthPerDay-2*122)/7) {
                        isDown=true;
            ...
            case MotionEvent.ACTION_UP:
                if(isDown) {
                    isDown=false;
                    isFirstDrawHeader=false;
                    mSelectedDayOfMonthListener.onSelectedDayOfMonth(selectedDayOfMonth);
                    invalidate();
                }
                break;
        }
        //ljt modify
        Calendar selectedTime = getTimeFromPoint(event.getX(), event.getY());
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                invalidate();
                if (mStartMoveLineTop != null && mEndMoveLineTop != null) {
                    mStartMoveLineTop = null;
                    mEndMoveLineTop = null;
                }
        ...
        //ljt modify
        if(isEventLongPress) {
            if (mEventLongPressListener != null && currentSelectEventRect != null) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_MOVE:
                        Calendar changeTime = getTimeFromPoint(event.getX(), event.getY());
                        mEventLongPressListener.onEventLongPress(currentSelectEventRect.originalEvent,
                                currentSelectEventRect.rectF,changeTime,isStartOrEndTime);
                        break;
                    case MotionEvent.ACTION_UP:
                        isEventLongPress=false;
                        break;
                }
            }
        }
        //ljt modify
        if(isEmptyLongPress) {
            if (mEmptyViewLongPressListener != null && event.getX() > mHeaderColumnWidth && event.getY() > (mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_MOVE:
                        if (selectedTime != null) {
                            mEmptyViewLongPressListener.onEmptyViewLongPress(selectedTime,startOrEnd);
                        ...
                        break;
                    case MotionEvent.ACTION_UP:
                        isEmptyLongPress=false;
                        if(startOrEnd) {
                            startOrEnd=false;
                            mEventDrawListener.onDrawEvent();
                        } else {
                            startOrEnd=true;
                        }
                        break;
    ...
    public interface EventLongPressListener {
         ...
         * @param eventRect: view containing the clicked event.
         */
        void onEventLongPress(WeekViewEvent event, RectF eventRect,Calendar currentChangeTime,int startOrEndTime);
    }
    ...
    public interface EmptyViewLongPressListener {
         ...
         */
        void onEmptyViewLongPress(Calendar time,boolean startOrEnd);
    }
    ...
    public interface SelectedDayOfMonthListener {
        void onSelectedDayOfMonth(int dayOfWeek);
    }
}
